#!/usr/bin/env python3
"""
test_hover_camera.py — Multi-altitude camera + LiDAR validation test.
...
-----
  ...
  python3 tests/test_hover_camera.py

  # Custom altitudes and timing
  python3 tests/test_hover_camera.py --altitudes 3 6 9 12 15 --hold 60 --pics 5

  # Quick test — 2 levels, 20s hold, 3 pics
  python3 tests/test_hover_camera.py --altitudes 3 6 --hold 20 --pics 3

  # Keep collision avoidance active (stops only watchdog, not main)
  python3 tests/test_hover_camera.py --keep-collision

  # Skip service management if already stopped manually
  python3 tests/test_hover_camera.py --no-service-stop
...
_ARDUCAM_LOG  = Path("/tmp/hover_test_arducam.log")
...
# ══════════════════════════════════════════════════════════════════════════════
# Bag recorder  (one per altitude level)
# ══════════════════════════════════════════════════════════════════════════════
...
class HoverCameraNode:
    ...
    def _wait_new_frame(self, timeout: float = 3.0) -> Optional[object]:
        """
        Wait for a frame that is genuinely newer than whatever is cached.

        Compares by ROS header timestamp, not object identity, so it works
        even if ArducamNode sends duplicate message objects.
        """
    ...
    def write_report(self) -> bool:
        ...
        lines = [
            "=" * 65,
            "  DronePi — Multi-Altitude Hover Camera Validation Report",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Multi-altitude hover camera + LiDAR validation test."
    )
    parser.add_argument(
    ...
    print("=" * 60)
    print("  DronePi — Multi-Altitude Hover Camera Test")
    ...
    try:
        ...
        if not _wait_topic("/arducam/image_raw", timeout=15.0):
            print(f"  [FAIL] ArducamNode not publishing after 15s")
            return

        print("\n[Setup 3/3] Initialising ROS 2 node...")
        flight    = HoverCameraNode(args)
